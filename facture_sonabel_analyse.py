...
def exporter_pdf(df):
    ...
    for _, row in df.iterrows():
        ligne = f"{row['Mois']} | Ancien: {row['Ancien']} | Nouveau: {row['Nouveau']} | kWh: {row['kWh']} | Écart: {row['Écart %']}% {row['Alerte']}"
...
def interface_streamlit():
    ...
    if historique.loc[historique.index[-1], "Alerte Tarifs"]:
        st.error("⚠️ Montant facturé supérieur de plus de 25% au théorique !")
